...
GROUP BY DEPT_CODE HAVING E.DEPT_CODE = DEPT_CODE)  "부서별 급여평균"
FROM EMPLOYEE E
WHERE EXISTS (SELECT 1 FROM EMPLOYEE WHERE JOB_CODE NOT IN('J1','J2','J3')
AND E.SALARY > (SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE GROUP BY DEPT_CODE HAVING E.DEPT_CODE = DEPT_CODE))
