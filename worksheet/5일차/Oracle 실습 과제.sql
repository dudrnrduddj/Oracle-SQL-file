...
(SELECT COUNT(*) FROM TB_STUDENT  WHERE S.DEPARTMENT_NO = DEPARTMENT_NO AND ABSENCE_YN = 'Y')|| '명' "휴학생 수"
FROM TB_STUDENT S
GROUP BY DEPARTMENT_NO
ORDER BY 1 ASC, DEPARTMENT_NO ASC;


--JOIN 이용 -- 문제점 : 0명이 1명으로 출력됨
SELECT D.DEPARTMENT_NO "학과 번호",
D.DEPARTMENT_NAME "학과 이름",
COUNT(STUDENT_NO) ||'명' "휴학생 수"
FROM TB_DEPARTMENT D
LEFT OUTER JOIN TB_STUDENT S
ON S.DEPARTMENT_NO = D.DEPARTMENT_NO 
AND S.ABSENCE_YN = 'Y'
--WHERE ABSENCE_YN = 'Y'
GROUP BY D.DEPARTMENT_NO, D.DEPARTMENT_NAME
ORDER BY 1 ASC, D.DEPARTMENT_NO ASC;
...
SELECT STUDENT_SSN_MODIFY, STUDENT_NAME,
(SELECT DEPARTMENT_NAME FROM TB_DEPARTMENT WHERE DEPARTMENT_NO = S.DEPARTMENT_NO), 
(SELECT AVG(POINT) FROM TB_GRADE  WHERE S.STUDENT_NO = STUDENT_NO) "평점" 
FROM TB_STUDENT S
WHERE ABSENCE_YN = 'N' AND (SELECT AVG(POINT) FROM TB_GRADE WHERE S.STUDENT_NO = STUDENT_NO) >= 4;


SELECT POINT FROM TB_GRADE  WHERE STUDENT_NO = 'A472021';
...
-- 18번-- 국어국문학과 총평점이 가장 높은 학생의 ...

SELECT STUDENT_NO, STUDENT_NAME, TRUNC(AVG(POINT),3) FROM TB_STUDENT
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
JOIN TB_GRADE USING(STUDENT_NO)
WHERE DEPARTMENT_NAME = '국어국문학과'
GROUP BY STUDENT_NO, STUDENT_NAME
HAVING TRUNC(AVG(POINT),3) =
(SELECT MAX(TRUNC(AVG(POINT),3)) FROM TB_GRADE 
...
--15번-- 누적 수강생수 탑 3 최근 3년
-- 과목번호, 과목이름, 누적수강생수

SELECT * FROM
(SELECT CLASS_NO "과목번호", CLASS_NAME "과목이름", 
(SELECT COUNT(*) FROM TB_GRADE WHERE C.CLASS_NO = CLASS_NO ) "누적수강생수"
FROM TB_CLASS C
WHERE (SELECT MAX(TERM_NO) FROM TB_GRADE WHERE C.CLASS_NO = CLASS_NO) > 200700
ORDER BY 3 DESC) WHERE ROWNUM < 4;
