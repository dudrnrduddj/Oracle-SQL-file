...
-- ALTER(오라클 객체)
-- ALTER를 이용한 제약 조건 추가, 수정, 이름 변겨
-- ALTER를 이용한 컬럼 추가, 컬럼 데이터타입 수정, 커럼명 수정, 삭제, 테이블명 수정
...
ALTER TABLE USER_GRADE MODIFY GRADE_DATE DATE;
-- 4. 테이블의 컬럼명 수정
...
-- 제약조건이름과 제약조건을 추가 (만약 이름을 주지 않으면 자동으로 부여됨)

ALTER TABLE USER_FOREIGN_KEY ADD CONSTRAINT FK_GRADE_CODE FOREIGN KEY(GRADE_CODE)
                                                        REFERENCES USER_GRADE(GRADE_CODE);
-- primary key 생성 후 가능 
        
-- 2. 제약조건 삭제
-- Foreign key부터 삭제해야 primary key 삭제 가능!
ALTER TABLE USER_GRADE DROP CONSTRAINT PK_GRADE_CODE;
ALTER TABLE USER_FOREIGN_KEY DROP CONSTRAINT FK_GRADE_CODE;
...
--- 예제 1) USER_FOREIGN_KEY 테이블에 USER_DATE DATE 컬럼을 추가해주세요
ALTER TABLE USER_FOREIGN_KEY ADD USER_DATE DATE;
--- 예제 2) USER_FOREIGN_KEY 테이블에 USER_DATE DATE 컬럼을 삭제해주세요
ALTER TABLE USER_FOREIGN_KEY DROP COLUMN USER_DATE;
--- 예제 3) USER_FOREIGN_KEY 테이블에 USER_DATE 컬럼의 자료형을 VARCHAR2(10)으로 바꿔주세요
ALTER TABLE USER_FOREIGN_KEY MODIFY USER_DATE VARCHAR(2);
--- 예제 4) USER_FOREIGN_KEY 테이블에 USER_DATE 컬럼의 이름을 REG_DATE로 변경해주세요
ALTER TABLE USER_FOREIGN_KEY RENAME COLUMN USER_DATE TO REG_DATE;
--- 예제 5) USER_FOREIGN_KEY 테이블의 이름을 USER_TBL로 바꿔주세요 (2가지)
