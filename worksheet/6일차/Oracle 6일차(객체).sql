...
/*
    1.1 개요
        ...
        - SELECT 쿼리의 실행 결과를 화면에 저장한 논리적인 가상의 테이블(실질적인 데이터를 저장하고
          있지 않지만 하나의 테이블처럼 사용 가능함)
          
          1.2 종류
          1.2.1 Stored View(저장 O) - 이름을 붙여서 저장함.
          1.2.2 Inline View(저장 X) - FROM절 뒤에 적는 서브쿼리
*/
--1.2.1 Stored View(저장 O) - 이름을 붙여서 저장함.
...
AS
SELECT EMP_ID, EMP_NAME, JOB_NAME, DEPT_TITLE, LOCAL_NAME FROM EMPLOYEE
...
-- 1.5.1 OR REPLACE
-- 수정하기 (OR REPLACE : 수정할때 사용하는 옵션)
CREATE OR REPLACE VIEW V_EMPLOYEE(사번, 이름, 부서코드, 직급코드)
AS SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE;

-- 1.5.2 FORCE/NOFORCE (ex CREATE OR REPLACE FORCE VIEW ...) 
...
AS SELECT EMP_ID, EMP_NAME, JOB_NAME, DEPT_TITLE, NVL((SELECT EMP_NAME FROM EMPLOYEE WHERE E.MANAGER_ID = EMP_ID),'-'), HIRE_DATE
FROM EMPLOYEE E 
...
-- 사용자는 데이터 딕셔너리의 내용을 직접 수정하거나 삭제할 수 있음.
-- 데이터 딕셔너리 안에는 중요한 정보가 많이 있기 때문에 사용자는 이를 활용하기
-- 위해서는 데이터 딕셔너리 뷰를 사용하게 됨.
-- 데이터 딕셔너리 뷰의 종류
-- 1. USER_XXX : 접속한 계정이 소유한 객체 등에 관한 정보르 조회함.
SELECT * FROM USER_VIEWS; -- 만든 view 목록
SELECT * FROM USER_CONSTRAINTS; -- 만든 constraint 목록
SELECT * FROM USER_SYS_PRIVS;
SELECT * FROM USER_ROLE_PRIVS;
-- 2. ALL_XXX : 접속한 계정이 권한부여 받은 것과 소유한 것에 관한 정보를 조회함.
SELECT * FROM ALL_TABLES;
SELECT * FROM ALL_VIEWS;
-- 3. DBA_XXX : 데이터베이스 관리자만 접근이 가능한 객체 등의 정보 조회
SELECT * FROM DBA_TABLES; -- 시스템 계정에서만 실행가능
...
-- 그런 역할을 하는 것이 바로 sequence임.

-- 2.1 개요
-- - 순차적으로 정수 값을 자동으로 생성하는 객체, 자동 전호 발생기(채번기)의 역할을 함.
-- - 사용법 : CREATE SEQUENCE 시퀀스명; -> 기본값으로 생성( 1부터 1씩 증가)
-- 2.2 Sequence 옵션
 -- 2.2.1 Minvalue : 발생시킬 최소값 지정
 -- 2.2.2.MaxVALUE : 발생시킬 치대값 지정
 -- 2.2.3 START WITH : 처음 발생시킬 시작값 지점, 기본값 1 
 -- 2.2.4 INCREMENT BY : 다음 값에 대한 증가치, 기본값 1
 -- 2.2.5 NOCYCLE : 시퀀스값이 최대값까지 증가를 완료하면 CYCLE은 START WITH로 다시 시작함.
 -- 2.2.6 NOCACHE : 메모리 상에서 시퀀스값을 관리, 기본값 20, NOCATCH를 안하면 갑자기 시퀀스값이 증가하게 됨
...
-- SELECT이 뒤 또는 INSERT INTO VALUES의 전달값으로 작성
...
ALTER TABLE ORDER_TBL ADD CONSTRAINT PK_ORDER_NO PRIMARY KEY (ORDER_NO);
ALTER TABLE ORDER_TBL MODIFY ORDER_DATE DEFAULT SYSDATE;
...
CREATE SEQUENCE SQ_KH_MEMBER_ID
START WITH 500
INCREMENT BY 10
NOCYCLE
NOCACHE;

CREATE SEQUENCE SQ_KH_MEMEBER_JOIN_NO
START WITH 1
INCREMENT BY 1
NOCYCLE
NOCACHE;
...
INSERT INTO KH_MEMBER VALUES(SQ_KH_MEMBER_ID.NEXTVAL, '홍길동', 20, SQ_KH_MEMEBER_JOIN_NO.NEXTVAL);
INSERT INTO KH_MEMBER VALUES(SQ_KH_MEMBER_ID.NEXTVAL, '청길동', 30, SQ_KH_MEMEBER_JOIN_NO.NEXTVAL);
INSERT INTO KH_MEMBER VALUES(SQ_KH_MEMBER_ID.NEXTVAL, '외길동', 40, SQ_KH_MEMEBER_JOIN_NO.NEXTVAL);
INSERT INTO KH_MEMBER VALUES(SQ_KH_MEMBER_ID.NEXTVAL, '고길동', 50, SQ_KH_MEMEBER_JOIN_NO.NEXTVAL);
...
-- 시퀀스 수정
-- ALTER SEQUENCE를 이용해 옵션을 다시 지정해줌
ALTER SEQUENCE SQ_KH_MEMBER_ID
