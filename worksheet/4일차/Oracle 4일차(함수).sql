...
SELECT FLOOR(SYSDATE - HIRED_DATE) FROM EMPLOYEE;

-- 1.1 단일행 함수의 종류
--  1.1.1 숫자 처리 함수
--      - ABS, MOD, TRUNC, FLOOR, ROUND, CEIL
--  1.1.2 문자 처리 함수
--        - LENGTH, LENGTHB : 길이 구함
--        - INSTR, INSTRB : 위치 구함
--        - LPAD, FPAD : 빈 곳을 채워줌
--        - LTRIM,/RTRIM : 특정문자 제거(공백 제거)
--        - TRIM
--        - SUBSTR : 문자열 잘라줌
--        - CONCAT : 문자열 합쳐줌
--        - REPLACE : 문자열 바꿔줌
--        사용법은 함수명(값1, 값2 ...)으로 적고 값에는 리터럴이나 
--  1.1.3 날짜 처리 함수
...
SELECT SYSDATE "입대일자", ADD_MONTHS(SYSDATE, 18) "제대일자",(ADD_MONTHS(SYSDATE, 18)-SYSDATE)*3||'일' "짬밥수" FROM DUAL;
...
--	ex) 노옹철	no_hc
--	ex) 정중하	jung_jh
SELECT EMP_NAME "이름", SUBSTR(EMAIL,0, INSTR(EMAIL,'@')-1) "아이디" FROM EMPLOYEE;
...
SELECT EMP_NAME "직원명", CONCAT('19',SUBSTR(EMP_NO, 1, 2)) "년생", NVL(BONUS,0) "보너스" FROM EMPLOYEE WHERE EMP_NO LIKE '6%';
...
--   사번 사원명 부서코드 입사일
SELECT EMP_ID "사번", EMP_NO "사원명", DEPT_CODE "부서코드", HIRE_DATE "입사일" FROM EMPLOYEE WHERE DEPT_CODE IN('D5', 'D9');

--8. 직원명, 입사일, 오늘까지의 근무일수 조회 
--	* 주말도 포함 , 소수점 아래는 버림
SELECT EMP_NAME "직원명", HIRE_DATE "입사일", TRUNC(SYSDATE-HIRE_DATE) "근무일수" FROM EMPLOYEE;
...
'19'||SUBSTR(EMP_NO,1,2)||'년 '||SUBSTR(EMP_NO,3,2)||'월'||SUBSTR(EMP_NO,4,2)||'일' "생년월일",
EXTRACT(YEAR FROM SYSDATE)-EXTRACT(YEAR FROM TO_DATE('19'||SUBSTR(EMP_NO,1,6),'YYMMDD')) "나이" FROM EMPLOYEE;


--10. 직원명, 직급코드, 연봉(원) 조회
--  단, 연봉은 ￦57,000,000 으로 표시되게 함
--     연봉은 보너스포인트가 적용된 1년치 급여임
SELECT EMP_NAME "직원명", NVL(DEPT_CODE,'X') "직급코드", TO_CHAR(SALARY*(12.3), 'L999,999,999') "연봉" FROM EMPLOYEE;
...
-- CASE 함수
-- 사용법 : CASE WHEN 변하는 값 = 조건값 THEN  참일때 값 WHEN 변하는값 = 조건값 THEN 참일때 값 ELSE 모두 아닐때 값 END
-- 예시 : CASE WHEN 변하는 값 = 1 THEN '빨강' WHEN 변하는 값 = 2 THEN '노랑' ELSE '파랑' END

SELECT EMP_NAME "이름", DECODE(SUBSTR(EMP_NO,8 ,1), 1, '남', 2, '여', 3, '남', 4, '여', 'X') "성별" FROM EMPLOYEE;
...
--실습1
--EMPLOYEE 테이블에서 부서코드, 그룹별 급여의 합계, 
--그룹별 급여의 평균(정수처리),인원수를 조회하고, 부서코드 순으로 정렬하세요
SELECT DEPT_CODE "부서코드", SUM(SALARY) "급여합계", FLOOR(AVG(SALARY)) "급여평균", COUNT(*) "인원수" FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;

-- 실습2
--EMPLOYEE 테이블에서 부서코드, 보너스를 지급받는 사원 수를 조회하고 부서코드 순으로 정렬하세요
SELECT DEPT_CODE "부서코드", COUNT(EMP_NAME) "사원 수" FROM EMPLOYEE
...
SELECT COUNT(EMP_NAME) "사원수", ROUND(AVG(SALARY)) "평균급여" FROM EMPLOYEE
--WHERE JOB_CODE <> 'J1'
--WHERE JOB_CODE != 'J1'
...
SELECT EXTRACT(YEAR FROM HIRE_DATE) "입사년도",COUNT(EMP_NAME) "입사년도별 인원수" FROM EMPLOYEE WHERE JOB_CODE <> 'J1'
...
TO_CHAR(ROUND(AVG(SALARY)),'999,999,999') "성별별 급여의 평균",
TO_CHAR(SUM(SALARY),'999,999,999') "성별별 급여의 합계",
COUNT(EMP_NAME) "인원수"
FROM EMPLOYEE
...
SELECT DEPT_CODE "부서",JOB_CODE "직급",SUM(SALARY) FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE
ORDER BY DEPT_CODE;

-- 실습7
-- 부서내 성별 인원수를 구하시오
SELECT DEPT_CODE, DECODE(SUBSTR(EMP_NO,8,1),1, '남', 2, '여',3,'남',4,'여', '무')"성별",COUNT(*) FROM EMPLOYEE
...
SELECT DEPT_CODE, COUNT(EMP_NAME) FROM EMPLOYEE GROUP BY DEPT_CODE
...
SELECT DEPT_CODE "부서", COUNT(*) "인원수" FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING COUNT(*) > 5;

-- 실습문제2
--부서내 직급별 인원수가 3명이상인 직급의 부서코드, 직급코드, 인원수를 출력하세요.
SELECT DEPT_CODE "부서코드", JOB_CODE "직급코드", COUNT(*) "인원수" FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE
HAVING COUNT(*) >= 3
ORDER BY 1 ASC;

-- 실습문제3
-- 매니져가 관리하는 사원이 두명이상인 매니져아이디와 관리하는 사원수를 출력하세요.
SELECT MANAGER_ID, COUNT(MANAGER_ID) FROM EMPLOYEE
GROUP BY MANAGER_ID
