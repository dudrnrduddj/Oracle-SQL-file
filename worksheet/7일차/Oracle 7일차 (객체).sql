...
-- 오라클 객체의 종류
-- 사용자(USER), 테이블(TABlE),  뷰(VIEW), 시퀀스(SEQUENCE), 인덱스(INDEX)
...
-- 사용자에게 권한을 부여할 때 한 개씩 부여하게 된다면 권한 부여 및 관리가 불편하므로 사용하는 객체임
SELECT * FROM DBA_SYS_PRIVS
ORDER BY 2 ASC; -- System계정 실행

-- *DCL(Data Control Language) : GRANT / REVOKE
-- 권한부여 및 회수는 관리자 세션에서 사용 가능
-- 관리자 계정
-- 1. system : 일반관리자, 대부분의 권한을 가지고 있음.
-- 2. sys : DB생성/삭제 권한이 있음. 로그인 옵션으로 as sysdba를 적어줘야 함.
...
-- CREATE VIEW 권한이 RESOURCE ROLE에 없었기 때문에 따로 권한부여 해줌
...
-- - KEY와 VALUE형태로 생성이 되며 KEY에는 인덱스로 만들 컬럼값, VALUE에는 행이 저장된 주소값이 저장됨
...
WHERE EMP_ID LIKE '2%';
SET TIMING OFF;
-- F10으로 실행하여 오라클 PLAN확인 가능, 튜닝 시 사용됨.
